...
public class MyProductsApiController extends MyProductsApiBareController implements ProductsApi
{
    ...
    @Override
    public ResponseEntity<Products> collectionsContainingProduct(String lidvid, @Valid Integer start,
            ...
            @Valid List<String> fields, @Valid List<String> sort, @Valid Boolean summaryOnly) throws IOException
    {
        ...
        try
        {
            ...
            {
                GetRequest request = new GetRequest(this.esRegistryConnection.getRegistryIndex(),
                        (String) hits.getAt(i).getSourceAsMap().get("collection_lidvid"));
                Map<String, Object> sourceAsMap = this.esRegistryConnection.getRestHighLevelClient()
                        .get(request, RequestOptions.DEFAULT).getSourceAsMap();
                Map<String, XMLMashallableProperyValue> filteredMapJsonProperties = ProductBusinessObject
                        .getFilteredProperties(sourceAsMap, fields,
                                new ArrayList<String>(Arrays.asList(ElasticSearchUtil
                                        .elasticPropertyToJsonProperty(EntitytProductWithBlob.BLOB_PROPERTY))));
