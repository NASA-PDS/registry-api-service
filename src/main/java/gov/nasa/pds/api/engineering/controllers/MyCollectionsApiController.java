...
public class MyCollectionsApiController extends MyProductsApiBareController implements CollectionsApi
{
    ...
    private Products getProductChildren(String lidvid, int start, int limit, List<String> fields, List<String> sort,
            boolean onlySummary) throws IOException, LidVidNotFoundException
    {
        ...
        try
        {
            ...
            CollectionProductRefBusinessObject collectionProductRefBO = new CollectionProductRefBusinessObject(
                    this.esRegistryConnection);
            CollectionProductRelationships collectionProductRelationships = collectionProductRefBO
                    .getCollectionProductsIterable(lidvid, start, limit);
            ...
            for (EntityProduct eProd : collectionProductRelationships)
            {
                if (eProd != null)
                {
                    ...
                    Map<String, XMLMashallableProperyValue> filteredMapJsonProperties = ProductBusinessObject
                            .getFilteredProperties(eProd.getProperties(), fields, null);
                ...
                {
                    MyCollectionsApiController.log
                            .warn("Couldn't get one product child of collection " + lidvid + " in elasticSearch");
    ...
    @Override
    public ResponseEntity<Products> bundlesContainingCollection(String lidvid, @Valid Integer start,
            @Valid Integer limit, @Valid List<String> fields, @Valid List<String> sort, @Valid Boolean summaryOnly)
    {
        String accept = this.request.getHeader("Accept");
        MyCollectionsApiController.log.info("accept value is " + accept);
    ...
    @SuppressWarnings("unchecked")
    private Products getContainingBundle(String lidvid, int start, int limit, List<String> fields, List<String> sort,
            boolean summaryOnly) throws IOException
    {
        ...
        MyCollectionsApiController.log
                .info("find all bundles containing the collection lid: " + lidvid.substring(0, lidvid.indexOf("::")));
        HashSet<String> uniqueProperties = new HashSet<String>();
        Products products = new Products();
        ...
        try
        {
            ...
            for (int i = start; start < limit && i < response.getHits().getHits().length; i++)
            {
                ...
                Map<String, XMLMashallableProperyValue> filteredMapJsonProperties = ProductBusinessObject
                        .getFilteredProperties(sourceAsMap, fields,
                                new ArrayList<String>(Arrays.asList(ElasticSearchUtil
                                        .elasticPropertyToJsonProperty(EntitytProductWithBlob.BLOB_PROPERTY))));
