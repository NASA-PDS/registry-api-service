...
public class MyBundlesApiController extends MyProductsApiBareController implements BundlesApi
{
            ...
            @ApiParam(value = "only return the summary, useful to get the list of available properties", defaultValue = "false") @Valid @RequestParam(value = "only-summary", required = false, defaultValue = "false") Boolean onlySummary)
    {
        ...
        req.setSearchInfo(q, keyword);
        req.setFieldInfo(fields, sort, onlySummary);
        req.setPageInfo(start, limit);

        return this.getProductsResponseEntity(req);
    }
    ...
    private Products getCollectionChildren(String lidvid, int start, int limit, List<String> fields, List<String> sort,
            boolean onlySummary) throws IOException
    {
        if (!lidvid.contains("::") && !lidvid.endsWith(":"))
        ...
        try
        {
            getBundleResponse = restHighLevelClient.get(getBundleRequest, RequestOptions.DEFAULT);
            ...
            if (getBundleResponse.isExists())
            {
                ...
                for (String collectionLid : collections)
                {
                    if ((i >= start) && (i < start + limit))
                    {
                        ...
                        GetRequest getCollectionRequest = new ElasticSearchRegistrySearchRequestBuilder()
                                .getGetProductRequest(collectionLidVid);

                        // GetRequest getCollectionRequest = new
                        // GetRequest(this.esRegistryConnection.getRegistryIndex(),
                        // collectionLidVid);

                        GetResponse getCollectionResponse = null;

                        getCollectionResponse = restHighLevelClient.get(getCollectionRequest, RequestOptions.DEFAULT);
                        ...
                        if (getCollectionResponse.isExists())
                        {
                            ...
                            Map<String, XMLMashallableProperyValue> filteredMapJsonProperties = ProductBusinessObject
                                    .getFilteredProperties(sourceAsMap, fields, null);

                            uniqueProperties.addAll(filteredMapJsonProperties.keySet());

                            if (!onlySummary)
                            {
                                EntityProduct entityCollection = objectMapper.convertValue(sourceAsMap,
                                        EntityProduct.class);

                                Product collection = ElasticSearchUtil.ESentityProductToAPIProduct(entityCollection,
                                        this.getBaseURL());
                                collection.setProperties(
                                        (Map<String, PropertyArrayValues>) (Map<String, ?>) filteredMapJsonProperties);
                        ...
                        {
                            MyBundlesApiController.log.warn("Couldn't get collection child " + collectionLidVid
                                    + " of bundle " + lidvid + " in elasticSearch");
                        }
                    }
                    i += 1;
    ...
    private ResponseEntity<Products> getBundlesCollections(String lidvid, int start, int limit, List<String> fields,
            List<String> sort, boolean onlySummary)
    {
    ...
    @SuppressWarnings("unchecked")
    private Products getProductChildren(String lidvid, int start, int limit, List<String> fields, List<String> sort,
            boolean onlySummary) throws IOException
    {
        ...
        try
        {
            ...
            if (getBundleResponse.isExists())
            {
                ...
                for (String collectionLid : collections)
                {
                    ...
                    // TODO change the request to get collections from their lidvid and not from the
                    // ID which is not right, we are missing packets
                    MyBundlesApiController.log.info("get collection with lidvid " + collectionLidVid);
                    GetRequest getCollectionRequest = new GetRequest(this.esRegistryConnection.getRegistryRefIndex(),
                            collectionLidVid);
                    GetResponse getCollectionResponse = restHighLevelClient.get(getCollectionRequest,
                    ...
                    if (getCollectionResponse.isExists())
                    {
                        ...
                        {
                            productLidvids.add((String) temp);
                        }
                        else
                        {
                            productLidvids.addAll((List<String>) temp);
                        }
                    }
                    else
                    {
                ...
                MyBundlesApiController.log
                        .info("total number of product lidvids " + Integer.toString(productLidvids.size()));
                for (int i = start; i < start + limit && i < productLidvids.size(); i++)
                {
                    ...
                    {
                        MyBundlesApiController.log.warn("Couldn't get product child " + productLidvids.get(i)
                                + " of bundle " + lidvid + " in elasticSearch");
                    }
                }
            }
